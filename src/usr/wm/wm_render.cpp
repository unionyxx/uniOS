...
void draw_window_client_clipped(Surface *dst, const Window &w, const DirtyRect &clip)
{
    ...
    // While the client catches up to an interactive resize, nearest-neighbor
    // stretch the committed content across the current client area instead of
    // anchoring it 1:1 and exposing a hole; it snaps back to crisp 1:1 once the
    // client commits the new size. Opaque, unscrolled windows only — the
    // anchored path below still serves transparent or scrolled surfaces.
    if (!w.transparent && w.resize_configure_pending && w.scroll_x == 0 && w.scroll_y == 0 &&
        w.client_committed_w > 0 && w.client_committed_h > 0 && inner_w > 0 && inner_h > 0 && inner_w <= 8192 &&
        (w.client_committed_w != inner_w || w.client_committed_h != inner_h) && w.client_committed_w <= w.buffer_w &&
        w.client_committed_h <= w.buffer_h && static_cast<int64_t>(inner_w) * 4 >= w.client_committed_w &&
        static_cast<int64_t>(inner_w) <= static_cast<int64_t>(w.client_committed_w) * 4 &&
        static_cast<int64_t>(inner_h) * 4 >= w.client_committed_h &&
        static_cast<int64_t>(inner_h) <= static_cast<int64_t>(w.client_committed_h) * 4) {
        static int32_t s_stretch_col_lut[8192];
        const int cw = w.client_committed_w;
        const int ch = w.client_committed_h;
        for (int dx = 0; dx < inner_w; dx++) {
            int32_t sx = static_cast<int32_t>((static_cast<int64_t>(dx) * cw) / inner_w);
            if (sx >= cw)
                sx = cw - 1;
            s_stretch_col_lut[dx] = sx;
        }
        ...
        for (int dy = dy0; dy < dy1; dy++) {
            int sy = static_cast<int>((static_cast<int64_t>(dy - inner_top) * ch) / inner_h);
            if (sy >= ch)
                sy = ch - 1;
            const uint32_t *src_row = &w.buffer[static_cast<size_t>(sy) * w.buffer_w];
