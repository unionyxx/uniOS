              ...
              uint8_t **buffer_out)
{
    ...
    wav_header.riff_descriptor = (struct WavRiffDescriptor *)file_data;
    wav_header.fmt_chunk = (struct WavFmtChunk *)(file_data + sizeof(struct WavRiffDescriptor));

    char *wave = (char *)wav_header.riff_descriptor->wave;
    if (wave[0] != 'W' || wave[1] != 'A' || wave[2] != 'V' || wave[3] != 'E') {
        free(file_data);
        LOG_ERROR("wav", "%s: invalid wav header", filename);
        return false;
    }
    ...
    struct WavDataChunk *data_chunk = wav_header.data_chunk;
    if (!data_chunk) {
        free(file_data);
    ...
    if (fmt_chunk->audio_format == 0 || fmt_chunk->samples == 0 || fmt_chunk->channels == 0 ||
        data_chunk->data_size == 0) {
        free(file_data);
        LOG_ERROR("wav", "%s: invalid wav data", filename);
        return false;
    }

    if (fmt_chunk->audio_format != 1) {
        free(file_data);
        LOG_ERROR("wav", "%s: non-pcm format is not supported", filename);
        return false;
    }

    if (fmt_chunk->bits_per_sample != 16) {
        free(file_data);
        LOG_ERROR("wav", "%s: only 16-bit data is supported", filename);
        return false;
    }
