...
[[nodiscard]] static uint16_t e1000_eeprom_read(uint8_t addr)
{
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        if (val & E1000_EERD_DONE)
            return static_cast<uint16_t>(val >> E1000_EERD_DATA_SHIFT);
...
static void e1000_read_mac()
{
    ...
    if (ral != 0 || (rah & 0xFFFF) != 0) {
        ...
        KLOG(LogModule::Net, LogLevel::Trace, "e1000: MAC from RAL/RAH: %02x:%02x:%02x:%02x:%02x:%02x", g_e1000.mac[0],
             g_e1000.mac[1], g_e1000.mac[2], g_e1000.mac[3], g_e1000.mac[4], g_e1000.mac[5]);
        return;
    }

    const uint16_t w0 = e1000_eeprom_read(0), w1 = e1000_eeprom_read(1), w2 = e1000_eeprom_read(2);
    g_e1000.mac[0] = w0 & 0xFF;
    g_e1000.mac[1] = (w0 >> 8) & 0xFF;
    g_e1000.mac[2] = w1 & 0xFF;
    g_e1000.mac[3] = (w1 >> 8) & 0xFF;
    g_e1000.mac[4] = w2 & 0xFF;
    g_e1000.mac[5] = (w2 >> 8) & 0xFF;

    KLOG(LogModule::Net, LogLevel::Trace, "e1000: MAC from EEPROM: %02x:%02x:%02x:%02x:%02x:%02x", g_e1000.mac[0],
         g_e1000.mac[1], g_e1000.mac[2], g_e1000.mac[3], g_e1000.mac[4], g_e1000.mac[5]);
    e1000_write_reg(E1000_REG_RAL0,
                    static_cast<uint32_t>(g_e1000.mac[0]) | (static_cast<uint32_t>(g_e1000.mac[1]) << 8) |
                        (static_cast<uint32_t>(g_e1000.mac[2]) << 16) | (static_cast<uint32_t>(g_e1000.mac[3]) << 24));
    e1000_write_reg(E1000_REG_RAH0,
                    static_cast<uint32_t>(g_e1000.mac[4]) | (static_cast<uint32_t>(g_e1000.mac[5]) << 8) | (1u << 31));
}
...
[[nodiscard]] static bool e1000_init_rx()
{
    ...
    e1000_write_reg(E1000_REG_RCTL,
                    E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_BSIZE_2048 | E1000_RCTL_SECRC | E1000_RCTL_LBM_NONE);
...
bool e1000_init()
{
    ...
    for (uint16_t b = 0; b < 256 && !found; b++) {
        for (uint8_t d = 0; d < 32 && !found; d++) {
            if (pci_config_read16(b, d, 0, PCI_VENDOR_ID) == 0xFFFF)
                continue;
            ...
            for (uint8_t f = 0; f < max_f && !found; f++) {
                ...
                if (pci_config_read8(b, d, f, PCI_CLASS) == 0x02 && pci_config_read8(b, d, f, PCI_SUBCLASS) == 0x00) {
                    nic = {static_cast<uint8_t>(b),
                           ...
                           pci_config_read8(b, d, f, PCI_HEADER_TYPE),
                           pci_config_read8(b, d, f, PCI_INTERRUPT_LINE)};
                    found = true;
    ...
    uint64_t bar_sz;
    uint64_t bar0 = pci_get_bar(&nic, 0, &bar_sz);
    ...
    if (!g_e1000.mmio_base) {
        DEBUG_ERROR("e1000: failed to map MMIO");
        return false;
    }
    KLOG(LogModule::Net, LogLevel::Trace, "e1000: MMIO base 0x%lx (phys 0x%lx), size %lu", g_e1000.mmio_base, bar0,
         bar_sz);
    e1000_write_reg(E1000_REG_CTRL, e1000_read_reg(E1000_REG_CTRL) | E1000_CTRL_RST);
    for (int i = 0; i < 100; i++) {
        for (volatile int j = 0; j < 10000; j++)
            ;
        if (!(e1000_read_reg(E1000_REG_CTRL) & E1000_CTRL_RST))
            break;
    }
    e1000_write_reg(E1000_REG_IMC, 0xFFFFFFFF);
    static_cast<void>(e1000_read_reg(E1000_REG_ICR));
    for (int i = 0; i < 128; i++)
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (e1000_read_reg(E1000_REG_STATUS) & E1000_STATUS_LU) {
            ...
            DEBUG_INFO("e1000: link UP");
            break;
        }
    }
    if (!g_e1000.link_up)
        DEBUG_WARN("e1000: link DOWN");
    g_e1000.initialized = true;
    DEBUG_INFO("e1000: initialized");
    return true;
}
