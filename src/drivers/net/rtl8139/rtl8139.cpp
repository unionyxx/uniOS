...
[[nodiscard]] static inline uint8_t rtl_inb(uint16_t reg)
{
    return inb(g_rtl8139.io_base + reg);
}
[[nodiscard]] static inline uint16_t rtl_inw(uint16_t reg)
{
    return inw(g_rtl8139.io_base + reg);
}
[[nodiscard]] static inline uint32_t rtl_inl(uint16_t reg)
{
...
static void rtl8139_read_mac()
{
    ...
    DEBUG_INFO("rtl8139: mAC %02x:%02x:%02x:%02x:%02x:%02x", g_rtl8139.mac[0], g_rtl8139.mac[1], g_rtl8139.mac[2],
...
bool rtl8139_init()
{
    ...
    for (uint16_t b = 0; b < 256 && !found; b++) {
        for (uint8_t d = 0; d < 32 && !found; d++) {
            if (pci_config_read16(b, d, 0, PCI_VENDOR_ID) != RTL8139_VENDOR_ID)
                continue;
            const uint8_t max_f = (pci_config_read8(b, d, 0, PCI_HEADER_TYPE) & 0x80) ? 8 : 1;
            for (uint8_t f = 0; f < max_f && !found; f++) {
                if (pci_config_read16(b, d, f, PCI_VENDOR_ID) != RTL8139_VENDOR_ID)
                    continue;
                ...
                if (pci_config_read8(b, d, f, PCI_CLASS) == 0x02 && pci_config_read8(b, d, f, PCI_SUBCLASS) == 0x00 &&
                    (dev_id == 0x8139 || dev_id == 0x8138 || dev_id == 0x8136)) {
    ...
    for (int i = 0; i < 10000; i++) {
        if (!(rtl_inb(RTL_REG_CMD) & RTL_CMD_RST))
            break;
...
{
    if (g_rtl8139.initialized)
        static_cast<void>(rtl_inw(RTL_REG_ISR));
}
