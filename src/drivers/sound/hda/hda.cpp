...
void hda_init()
{
    ...
    while (!(mmio_read16((void *)(hda_info.base + HDA_GLOBAL_CONTROL)) & HDA_GLOBAL_CONTROL_IN_OPERATIONAL_STATE)) {
        io_wait();
        if (--reset_timeout == 0) {
            DEBUG_ERROR("controller failed to reach operational state");
            return;
        }
    }
    ...
    // So to get address of output stream descriptor we need to skip input stream descriptors by adding input stream
    // count multiplied by stream descriptor size.
    ...
    hda_info.output_stream =
        hda_info.base + HDA_STREAM_DESCRIPTOR_BASE + HDA_STREAM_DESCRIPTOR_SIZE * input_stream_count;
    ...
    // Tell sound card how many entries CORB/RIRB have. We assume that every sound card has 256 entries, so set 256
    // entries.
    ...
    while (mmio_read16((void *)(hda_info.base + HDA_CORB_READ_POINTER)) & HDA_CORB_READ_POINTER_IN_RESET) {
        io_wait();
        if (--corb_timeout == 0) {
            DEBUG_WARN("hda: CORB read pointer clear timeout");
            break;
        }
    }
    ...
    for (uint32_t node = first_node; node < last_node; node++) {
        ...
        if ((hda_send_command(hda_info.codec, node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_FUNCTION_GROUP_TYPE) &
             0x7F) == 0x1) {
    ...
    hda_info.afg.supported_rates = hda_send_command(hda_info.codec, hda_info.afg.node, HDA_VERB_GET_PARAMETER,
                                                    HDA_NODE_PARAMETER_SUPPORTED_PCM_RATES);
    hda_info.afg.supported_formats = hda_send_command(hda_info.codec, hda_info.afg.node, HDA_VERB_GET_PARAMETER,
                                                      HDA_NODE_PARAMETER_SUPPORTED_FORMATS);

    hda_info.afg.output_amplifier_capabilities = hda_send_command(
        hda_info.codec, hda_info.afg.node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_OUTPUT_AMPLIFIER_CAPABILITIES);

    // AFG is initialized now. Initialize AFG nodes.

    // Get node count from AFG.
    node_count =
        hda_send_command(hda_info.codec, hda_info.afg.node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_NODE_COUNT);

    // Failsafe: if the controller is offline or returned garbage data, abort cleanly
    if (node_count == 0xFFFFFFFF || node_count == 0) {
        DEBUG_ERROR("HDA controller returned invalid node count! Aborting HDA init.");
        return;
    }
    ...
    for (uint32_t node = first_node; node < last_node; node++) {
        // CRITICAL: Prevent array out-of-bounds memory corruption
        if (node >= HDA_MAX_AFG_NODES) {
            DEBUG_WARN("Skipping out-of-bounds HDA node: %d", node);
            continue;
        }
        ...
        uint8_t node_type = (hda_send_command(hda_info.codec, node, HDA_VERB_GET_PARAMETER,
                                              HDA_NODE_PARAMETER_AUDIO_WIDGET_CAPABILITIES) >>
                             20) &
                            0xF;

        // Get parent node and type of parent node of this pin.
        uint32_t parent_node = hda_get_node_connection_entry(current_node, 0);
        uint32_t parent_node_type = (hda_send_command(hda_info.codec, node, HDA_VERB_GET_PARAMETER,
                                                      HDA_NODE_PARAMETER_AUDIO_WIDGET_CAPABILITIES) >>
                                     20) &
                                    0xF;
        ...
        if (node_type == HDA_WIDGET_AUDIO_OUTPUT) {
            ...
            uint8_t pin_type =
                (hda_send_command(hda_info.codec, node, HDA_VERB_GET_PIN_WIDGET_CONFIGURATION, 0) >> 20) & 0xF;
...
uint16_t hda_get_node_connection_entry(HdAudioNode *node, uint32_t connection_entry_number)
{
    ...
    uint32_t list_length =
        hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_CONNECTION_LIST_LENGTH);
    ...
    // Short form?
    if (!(list_length & 0x80)) {
        return ((hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_CONNECTION_LIST_ENTRY,
                                  ((connection_entry_number / 4) * 4)) >>
                 ((connection_entry_number % 4) * 8)) &
                0xFF);
    }

    return ((hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_CONNECTION_LIST_ENTRY,
                              ((connection_entry_number / 2) * 2)) >>
             ((connection_entry_number % 2) * 16)) &
            0xFFFF);
}
...
void hda_init_pin(HdAudioNode *node)
{
    ...
    hda_send_command(hda_info.codec, node->node, HDA_VERB_SET_PIN_WIDGET_CONTROL,
                     (hda_send_command(hda_info.codec, node->node, 0xF07, 0) | 0x80 | 0x40));

    // Collect node capabilities.
    node->supported_rates =
        hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_SUPPORTED_PCM_RATES);
    node->supported_formats =
        hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_PARAMETER, HDA_NODE_PARAMETER_SUPPORTED_FORMATS);

    node->output_amplifier_capabilities = hda_send_command(hda_info.codec, node->node, HDA_VERB_GET_PARAMETER,
                                                           HDA_NODE_PARAMETER_OUTPUT_AMPLIFIER_CAPABILITIES);
...
void hda_set_node_volume(HdAudioNode *node, uint32_t volume)
{
    ...
    DEBUG_INFO("set node %d | %d volume to %d (amp capabilities: %p)", node->node, node->node_type, volume,
...
void hda_set_volume(uint8_t volume)
{
    ...
    for (uint32_t node = 0; node < HDA_MAX_AFG_NODES; node++) {
        HdAudioNode *current_node = &hda_info.nodes[node];
        // Skip uninitialized nodes (node ID is 0 or node type is 0 for uninitialized slots)
        if (current_node->node == 0 && current_node->node_type == 0)
            continue;
...
void hda_play(uint8_t *data, uint32_t size)
{
    ...
    // Wait for reset with timeout.
    uint32_t stream_timeout = 100000;
    while (!(mmio_read8((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_STREAM_CONTROL_1)) &
             HDA_STREAM_CONTROL_STREAM_IN_RESET)) {
        io_wait();
        if (--stream_timeout == 0) {
            DEBUG_WARN("stream reset timeout");
            break;
        }
    }
    ...
    while (mmio_read8((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_STREAM_CONTROL_1)) &
        ...
        io_wait();
        if (--stream_timeout == 0) {
            DEBUG_WARN("stream stop timeout");
            break;
        }
    }
    ...
    mmio_write8((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_STREAM_CONTROL_1),
                HDA_STREAM_CONTROL_STREAM_RUNNING);
...
void hda_stop()
{
    ...
    while (mmio_read8((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_STREAM_CONTROL_1)) &
        ...
        io_wait();
        if (--stop_timeout == 0) {
            DEBUG_WARN("hda: stream stop timeout");
            break;
        }
    }
    ...
    while (!(mmio_read8((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_STREAM_CONTROL_1)) &
        ...
        io_wait();
        if (--reset_timeout == 0) {
            DEBUG_WARN("hda: stream reset timeout");
            break;
        }
    }
...
void hda_poll()
{
    ...
    if (!hda_info.is_playing || hda_info.is_paused) {
        return;
    }
    ...
    // Get current stream position directly (inline for speed)
    uint32_t stream_pos = mmio_read32((void *)(hda_info.output_stream + HDA_STREAM_DESCRIPTOR_BUFFER_ENTRY_POSITION));
    ...
    if (stream_curr_buffer_entry == 0 && hda_info.current_buffer_entry > 0) {
        ...
        uint32_t src_offset = HDA_BUFFER_ENTRY_SOUND_BUFFER_SIZE *
    ...
    hda_info.played_bytes =
        (hda_info.buffer_entry_offset * HDA_BUFFER_ENTRY_COUNT * HDA_BUFFER_ENTRY_SOUND_BUFFER_SIZE) + stream_pos;
}
...
void hda_record_start(uint8_t *buffer, uint32_t size)
{
    ...
    mmio_write64((void *)(hda_info.input_stream + HDA_STREAM_DESCRIPTOR_BDL_BASE_ADDRESS),
                 hda_info.input_buffer_entries_dma.phys);
    mmio_write32((void *)(hda_info.input_stream + HDA_STREAM_DESCRIPTOR_RING_BUFFER_LENGTH),
                 HDA_BUFFER_ENTRY_SOUND_BUFFER_SIZE * HDA_BUFFER_ENTRY_COUNT);
