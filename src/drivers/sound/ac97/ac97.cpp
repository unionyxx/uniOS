...
void ac97_reset()
{
    ...
    if (ac97_info.free_sound_data_on_stop && ac97_info.buffer_to_free) {
        free(ac97_info.buffer_to_free);
    }
    ac97_info.free_sound_data_on_stop = false;
    ac97_info.buffer_to_free = nullptr;
...
void ac97_play_wav_file(const char *filename)
{
    ...
    // wav_open allocates memory for wav_buffer, so we should let the driver free it later
    ac97_info.free_sound_data_on_stop = true;
    ac97_info.buffer_to_free = wav_buffer;
...
void ac97_play_pcm_file(const char *filename)
{
    ...
    // PCM files don't have headers, so we assume a default rate if not already configured.
    // The user can change this via SYS_SOUND_CONFIG before calling play.
    extern uint32_t g_sound_sample_rate;
    if (g_sound_sample_rate == 0)
        g_sound_sample_rate = 44100;
    ac97_set_sample_rate(static_cast<uint16_t>(g_sound_sample_rate));

    // Play it!
    // ac97_play will use this data during polling, so let it free after stop
    ac97_info.free_sound_data_on_stop = true;
    ac97_info.buffer_to_free = data_ptr;
    ac97_play(data_ptr, data_size);
}
...
void ac97_poll()
{
    ...
    outb(ac97_info.nabm + AC97_NABM_PCM_OUT_LAST_VALID_ENTRY,
         (stream_curr_buffer_entry - 1) & (AC97_BUFFER_ENTRY_COUNT - 1));
    ...
    if (stream_curr_buffer_entry == 0 && ac97_info.current_buffer_entry > 0) {
        ...
        uint32_t src_offset = AC97_BUFFER_ENTRY_SOUND_BUFFER_SIZE *
    ...
    ac97_info.played_bytes =
        (ac97_info.buffer_entry_offset * AC97_BUFFER_ENTRY_COUNT * AC97_BUFFER_ENTRY_SOUND_BUFFER_SIZE) + stream_pos;
}
...
uint32_t ac97_get_stream_position()
{
    ...
    return current_buffer * AC97_BUFFER_ENTRY_SOUND_BUFFER_SIZE +
           (AC97_BUFFER_ENTRY_SOUND_BUFFER_SIZE / 2 - buffer_offset);
}
