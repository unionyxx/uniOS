...
void serial_init_port(uint16_t port, uint32_t baud)
{
    ...
    // Set divisor (lo byte)
    outb(port + SERIAL_DIVISOR_LOW, (uint8_t)(divisor & 0xFF));

    // Set divisor (hi byte)
    outb(port + SERIAL_DIVISOR_HIGH, (uint8_t)((divisor >> 8) & 0xFF));
    ...
    if (inb(port + SERIAL_DATA) != 0xAE) {
        // Serial port might not support loopback, but continue anyway
    }
...
bool serial_has_char()
{
    if (!serial_initialized)
        return false;
    return (inb(active_port + SERIAL_LINE_STATUS) & 0x01) != 0;
}
...
char serial_getc()
{
    if (!serial_has_char())
        return 0;
    char c = (char)inb(active_port + SERIAL_DATA);
    if (c == '\r')
        return '\n'; // Normalize CR to LF
    return c;
}
...
void serial_putc(char c)
{
    ...
    int timeout = 100000;
    while (!serial_is_ready() && timeout-- > 0) {
        asm volatile("pause");
    }

    if (timeout > 0) {
        outb(active_port + SERIAL_DATA, (uint8_t)c);
    }
}
    ...
    while (*fmt && bi < 511) {
        if (*fmt == '%') {
            ...
            switch (*fmt) {
                ...
                case 'd':
                case 'i': {
                    int64_t n = is_long ? va_arg(args, int64_t) : va_arg(args, int);
                    if (n < 0) {
                        buf[bi++] = '-';
                        n = -n;
                    }
                ...
                case 'u': {
                    uint64_t n = is_long ? va_arg(args, uint64_t) : va_arg(args, unsigned int);
                    if (n == 0) {
                        if (bi < 511)
                            buf[bi++] = '0';
                        break;
