...
| **preferences** | Settings app: Appearance (theme, wallpaper, transparency, animations), Desktop (grid, clock seconds, volume), Network (ethernet + DHCP toggles), System (launch terminal, storage mode). Writes registry fields and persists `SYSTEM.CFG`/wallpaper config. Wallpaper Apply/Default use release-to-apply with pressed feedback, the volume slider drags past the window via the WM pointer grab, Esc leaves the wallpaper field, and sticky panels redraw on scroll. Publishes app menus: Help (tips overlay, About). |
| **latitude** | Text/code editor: 2048 x 512 buffer, 512 KiB open limit with binary sniffing, syntax highlighting (text, C++, JS, Python, Rust, HTML, CSS, JSON, Markdown, shell), project browser, outline, search panes. New/Reload require a confirming second click while changes are unsaved, saves post a system toast, the titlebar tracks the open file, and hover clears on pointer exit. Mouse drag and Shift+arrows select text; Cut/Copy/Paste go through the system clipboard and a bounded line-snapshot undo/redo stack (typing bursts coalesce); accelerators Ctrl+Z/Y/A/X/C/V. Publishes app menus: File (New/Open/Save/Reload), Edit (Undo/Redo/Cut/Copy/Paste/Delete/Select All), Help (tips overlay, About). |
| **clock** | Analog clock with continuous-sweep hands driven by a free-running monotonic counter (TSC when available, otherwise scheduler ticks); an RTC phase servo trims it once per second through a bounded-rate correction (hand speed kept within 0.7x-1.3x) so the hands never snap, freeze, or reverse — only a genuine wall-clock step (>2 s) produces a single visible jump, and a non-`+1` RTC reading is accepted only after two consecutive matching reads. The tick rate is self-calibrated from observed RTC boundaries (with re-lock confirmation) and also paces the frame loop. Digital time and date below; per-frame damage is limited to the face and text regions. Publishes app menus: Help (tips overlay, About). |
| **calendar** | Month grid with today highlight, Sakamoto weekday computation, leap-year-aware month lengths. Prev/next month arrows, a Today/Go-to-Today button, wheel and Page Up/Down month navigation, arrow-key day selection, and hover that clears on pointer exit. Publishes app menus: Help (tips overlay, About). |
| **calculator** | Keypad calculator with pending-op accumulator, percent, sign toggle, and 15-place decimal cap. Full keyboard input (digits, operators, Enter/=, Backspace clears entry, Esc clears all), release-to-apply button semantics with drag-away cancel, an armed-operator highlight, divide-by-zero Error state, and a display that keeps the newest digits visible on overflow. Publishes app menus: Edit (Copy Result to the clipboard with Ctrl+C, Clear Entry, Clear All), Help (tips overlay, About). |
| **imageviewer** | Image viewer over libmedia (PNG, baseline JPEG, GIF, BMP, QOI). At startup it takes a pending open request from the registry (`open_generation`/`open_path`, consumed and cleared once); without one it shows an empty-state placeholder. The decoded image is fit to the window preserving aspect ratio through a cached bilinear-scaled copy, blended over a checkerboard so alpha stays visible, and the titlebar carries `name - WxH`. Load/decode failures show an error state. Publishes app menus: Help (About). |
| **about** | System information: kernel commit, bootloader name/version, CPU count, timer rate, memory totals, uptime, display capabilities. Double-buffered rendering; the 1 Hz refresh republishes only the runtime and memory panels unless structural data changes. Publishes a minimal Help menu whose only entry is the reserved About command the menubar handles itself (re-focusing this window). |
