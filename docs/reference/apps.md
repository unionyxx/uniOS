...
| **clock** | Analog clock with continuous-sweep hands driven by a monotonic counter (TSC when available, otherwise scheduler ticks calibrated against observed RTC second boundaries); the RTC re-anchors the phase once per second through a rate-limited ramp so the hands never snap. Digital time and date below; per-frame damage is limited to the face and text regions. |
