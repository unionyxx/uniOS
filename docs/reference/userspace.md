...
- **Window protocol**: registration, resize handling, damage commits — see [Window manager](wm.md). Regular (non-`WIN_FLAG_SYSTEM`) windows register with `protocol_version = 1` and use a two-slot client mailbox: the app renders into the slot the compositor is not presenting, then publishes it (`gui_window_mailbox_target` / `gui_window_mailbox_commit`), so the WM never reads a buffer mid-redraw. Shell surfaces keep the legacy single-buffer path.
