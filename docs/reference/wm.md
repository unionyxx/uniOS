...
While a resize is in flight the WM geometry changes before the client commits, so the compositor tracks each window's last committed content size (`client_committed_w/h`, updated on adoption and every resize ack, clamped to the mapped backing so an ack can never claim content the buffer does not hold) and only blits that region — never the slack pixels of an over-allocated backing. When a new configure generation is posted the WM copies the committed frame into a per-window snapshot (`resize_snapshot`, freed on close or once the gesture is acknowledged with no drag active). Until the client acks, opaque unscrolled windows nearest-neighbor stretch that snapshot across the whole client area (`wm::stretch_map_coord`, shared with the ktest suite), so content tracks the dragged edges exactly every frame instead of lagging behind them with a background band; the moment the client commits, the stretch collapses to a crisp 1:1 blit with no snap. Rendering from the WM-owned snapshot keeps the client's in-flight redraw from tearing the stretched frame. Transparent or scrolled windows and snapshot allocation failures fall back to the anchored 1:1 blit: committed content pins to the corner opposite the dragged edges, and the anchor survives pointer release until the outstanding configure is acknowledged. Shell surfaces (menubar, dock) bypass the configure protocol and redraw their whole entry, so their committed size tracks the registry geometry directly (bounded by the backing); this lets the menubar grow its window to expose the system dropdown.
...
- `SYSTEM_FLAG_WM_BENCH_DRAG` / `SYSTEM_FLAG_WM_BENCH_RESIZE` run a scripted 1000-frame benchmark: the WM drives synthetic window translation (drag) or geometry oscillation (resize) on the first visible user window, measures compose/present/frame costs and dirty area, logs a summary line, restores the window, and clears the flag. The resize mode routes every step through `set_window_bounds`, so it exercises the real configure/ack/snapshot-stretch pipeline; the summary additionally reports `resize stretch`/`fallback` draw counts, and a correct run shows stretch draws with zero fallback (fill-band) draws. No manual input is needed, so deltas are reproducible on hardware.
