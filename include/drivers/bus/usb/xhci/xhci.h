...
#define XECP_ID_USB_OWNERSHIP 1
#define XECP_ID_PROTOCOLS 2
#define XECP_ID_POWER 3
#define XECP_ID_VIRT 4

#define USBLEGSUP_FIRMWARE_SEM (1 << 16)
#define USBLEGSUP_OS_SEM (1 << 24)
#define USBLEGCTLSTS_SMI_ENABLE 0xFFFF0000
...
#define PORTSC_OCC (1 << 19)          // Over-current Change (OCC)
...
// NOTE: Layout is CSZ dependent. Use xhci_get_device_*_ctx helpers.
...
// NOTE: Layout is CSZ dependent. Use xhci_get_input_*_ctx helpers.
struct InputContext
{
...
struct XhciController
{
    ...
    uint8_t port_protocols[256]; // Protocol version (2 for USB2, 3 for USB3) for each port (1-based index)
    ...
    void (*intr_callbacks[256][32])(uint8_t slot_id, uint8_t ep_num, void *data, uint16_t length);

    // One-shot synchronous transfers for bulk/control-like class drivers.
    volatile bool sync_pending[256][32];
    volatile bool sync_complete[256][32];

    volatile bool command_event_ready;
    Trb last_command_event;
    volatile bool control_event_ready[256];
    Trb last_control_event[256];

    // Port status tracking
    volatile bool port_needs_enumeration[256];
};
...
// Callback type for interrupt transfers
typedef void (*XhciInterruptCallback)(uint8_t slot_id, uint8_t ep_num, void *data, uint16_t length);
...
bool xhci_address_device(uint8_t slot_id, uint8_t port, uint8_t speed, uint8_t parent_hub_slot = 0,
...
bool xhci_configure_endpoint(uint8_t slot_id, uint8_t ep_num, uint8_t ep_type, uint16_t max_packet, uint8_t interval,
                             uint8_t max_burst = 0, uint8_t mult = 0, uint16_t max_esit_payload = 0);
...
bool xhci_control_transfer(uint8_t slot_id, uint8_t request_type, uint8_t request, uint16_t value, uint16_t index,
                           uint16_t length, void *data, uint16_t *transferred, bool log_error = true);
bool xhci_bulk_transfer(uint8_t slot_id, uint8_t ep_num, bool in, void *data, uint32_t length,
                        uint32_t *transferred = nullptr, uint32_t timeout_ms = 5000);
